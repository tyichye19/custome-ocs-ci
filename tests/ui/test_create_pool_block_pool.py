...
from ocs_ci.helpers.helpers import create_ocs_object_from_kind_and_name, remove_ocs_object_from_list
...
class TestPoolUserInterface(ManageTest):
    ...
    def teardown(self, request, setup_ui):
        def finalizer():
            """
            The function take a global array need_to_delete which was appended while the test was running and
            tries to delete the object inside it first from UI and if fails from CLI.
            """
            if len(need_to_delete) > 0:
                need_to_delete.reverse()
                for ocs_data_to_delete in need_to_delete:
                    if ocs_data_to_delete.kind == constants.CEPHBLOCKPOOL:
                        logger.info(
                            f"Going to delete {ocs_data_to_delete.name} of kind {ocs_data_to_delete.kind} from UI"
                        )
                        bp_ui_object = BlockPoolUI(setup_ui)
                        if not bp_ui_object.delete_pool(ocs_data_to_delete.name):
                            logger.info(f"UI deletion of {ocs_data_to_delete.kind} with name {ocs_data_to_delete.name} failed")
                            logger.info(
                                f"Going to delete {ocs_data_to_delete.name} of kind {ocs_data_to_delete.kind} from cli"
                            )
                            ocs_data_to_delete.delete()
                    if ocs_data_to_delete.kind == constants.STORAGECLASS:
                        sc_ui_object = StorageClassUI(setup_ui)
                        logger.info(
                            f"Going to delete {ocs_data_to_delete.name} of kind {ocs_data_to_delete.kind} from UI"
                        )
                        if not sc_ui_object.delete_rbd_storage_class(
                            ocs_data_to_delete.name
                        ):
                            logger.info(
                                f"UI deletion of {ocs_data_to_delete.kind} with name {ocs_data_to_delete.name} failed")
                            logger.info(
                                f"Going to delete {ocs_data_to_delete.name} of kind {ocs_data_to_delete.kind} from cli"
                            )
                            ocs_data_to_delete.delete()
    ...
    def test_create_delete_pool(
        self, setup_ui, replica, compression, pvc_factory, pod_factory, project_factory
    ):
        """
        ...
        if pool_status:
            ...
            if pool_name:
                need_to_delete.append(
                    create_ocs_object_from_kind_and_name(
                        kind="CephBlockPool", resource_name=pool_name
                    )
                )
        ...
        # Creating project for PVC and POD
        proj_obj = project_factory()

        # Converting the storageclass name to python object
        sc_ocs_obj = create_ocs_object_from_kind_and_name(
            kind=constants.STORAGECLASS, resource_name=sc_name
        )
        ...
        if storageclass_object.delete_rbd_storage_class(sc_name):
            ...
            need_to_delete = remove_ocs_object_from_list(
                resource_name=sc_name,
                kind=constants.STORAGECLASS,
                object_list=need_to_delete,
            )
        else:
        ...
        if block_pool_ui_object.delete_pool(pool_name):
            ...
            raise PoolNotDeleted(f"Pool {pool_name} was not deleted successfully")
